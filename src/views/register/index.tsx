...
class Register extends React.Component<
  { history: any; location: any; match: any },
  {
    loading: boolean;
...
> {
  ...
  private _subUser: Subscription | null = null;

  componentDidMount() {
    historyService.on(window.location.pathname);
    this._subUser = authService.behaviorSubjectUser.subscribe((user: User|null) =>
      this.setState({
        contributor:  user ? { ...this.state.contributor, email: user.email } : this.state.contributor
      })
    );

    const id: string = this.props.match.params.id;
    eventStore.load().then((events: My.Event[]) => {
      ...
      EventStore.update(event)
        .then(() => {
          ...
          pwaService.notify(
            ...
            `✅ Rendez-vous le ${new Date(
              event.scheduled
            ).toLocaleString()} pour "${event.title}"`
          );
        })
        .catch(() => {
  ...
  onClickRegister(e: any) {
    ...
    if (
      whitelist.indexOf(e.target.tagName) > -1 ||
      whitelist.indexOf(e.target.parentElement.nodeName) > -1
    ) {
  ...
  render() {
    ...
    return (
      <div className="register">
        ...
        {(this.state.event === null || this.state.loading) && (
        ...
        >
          ...
          <div className="app-formcontrol">
            <TextField
              ...
              required
              InputProps={{
                readOnly: !!this.state.contributor.email,
                startAdornment: (
                  <InputAdornment position="start">
                    <AlternateEmailIcon />
                  </InputAdornment>
          ...
          {e && e.mode === "REMOTE_AND_PHYSICAL_CONF" && (
            <div className="app-formcontrol">
              <FormControlLabel
                id="faceToFace"
                control={
            ...
            <FormControlLabel
              ...
              control={
                <Checkbox
                  ...
                  required
                  onChange={(e) =>
                    this.setState({ acceptRecord: e.target.checked })
                  }
                  name="acceptRecord"
                  color="primary"
                />
